...
- TCP/IPにおける通信の流れ
    ...
    - 1の処理
        - ブラウザでURLを打ち込んでWebページにアクセスしてindex.htmlを表示するリクエストを送る
    - 2の処理
        - HTTPはTCPが使われる
            - OSのカーネルでHTTPの場合はTCPを使うように実装されているのだろう（多分）
            - OSのカーネルで実装されている3way handshakeが実行される。
                - https://www.slideshare.net/pandax381/tcpip-105857327
    - 3
    - P85の画像

    - パケットの流れ
        - アプリケーション層,プレゼンテーション層,セッション層: アプリケーションプログラムに実装
        ...
        - TCPはアプリケーションのプログラムの指示によってコネクションを確立/データの送信/切断 を行っている
            - go言語だとnetにAcceptとCloseでコネクションの確立/切断、ReadとWriteでデータの受け付けと送信を行っている
            - https://coderwall.com/p/yjoauq/go-tcp
