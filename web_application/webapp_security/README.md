...
# あとで書くこと
# 認証を実装するときに気を付けるべきセキュリティ項目

## ユーザ認証の種類
- Basic認証(HTTP Basic認証)、Digest認証(HTTP Digest認証)
- oauth、oauth2
    - https://qiita.com/TakahikoKawasaki/items/e37caf50776e00e733be
- カスタム定義認証
    - django(python)で認証
        - 「django-allauth」を使うと「ログイン、ログアウト、パスワード変更、パスワード再設定、ユーザ登録」がまとめられている
        - https://sinyblog.com/django/django-allauth/
    - go言語で実装(フレームワークを使わない)
        - 本番環境では自作はダメ。あくまでも勉強目的で
        - 以下のセキュリティ項目の対処法も大体フレームワークに入っているので、自作をする前にフレームワークにあるか確認する
        - goを使ったwebアプリケーション開発の本に書いてあると思う
...
- ログアウト処理は副作用があるのでPOSTメソッドでリクエストする
    - ログアウトボタンを押したときにPOSTでリクエストするということ

- ログアウト処理ではセッションを破棄する
    - 多分セッション用のデータベースから削除するということだと思う
...
## ユーザ登録
- メールアドレスの受信確認
    - 登録されたメールアドレスが他人のアドレスだったら大問題
        - パスワードリセットやアカウントロックされた時に使うメールアドレスだから、絶対に間違ってはいけない
    
    - 実現方法(1または2)
        1. メールにトークン付きURLを添付して、そのURLから処理を継続する
            - P496画像

        2. メールアドレスを入力した後、トークン(確認番号)入力画面に遷移する。トークンは、指定したメールアドレスにメール送信される
            - P497画像
...
- ユーザの自動登録の対処
    - botを使って大量に新規ユーザを作られてしまうことに対する対処法
        - 例えば、メールアドレスを大量に発行されてしまい、迷惑メールの発信元アドレスに使われること
    
    - 実現方法
        - CAPTCHA
            - 歪めた文字をユーザに入力させるやつ
        - reCAPTCHA
            - 私はロボットではありませんにチェックをつけるだけのやつ
            - Googleが開発
            - いろいろな言語から使うためのライブラリがあるらしい
        - パズル認証
            - パズルを完成させるやつかな？

## パスワード変更
- 現在のパスワードを確認させる
    - 現パスワードと新パスワードと新パスワード(確認)の3つを入力させる
        - セッションハイジャックされた状態でパスワードを変更されることを防ぐ
        - 現パスワードを入力させること(再認証)でCSRF対策にもなる

- パスワード変更時にメール送信させる
    - 重要な処理がなされた時にはメールに送信させる
    - 不正な操作に即座に気づくことができる

## メールアドレスの変更
- 新規メールアドレスに対する受信確認
    - 間違っていないかの確認
- 現メールアドレスと新メールアドレスと新メールアドレス(確認)の3つを入力させる
    - 「パスワード変更」と同じように、セッションハイジャックとCSRF対策

## パスワードリセット
- 管理者向けパスワードリセット機能
    1. 問い合わせを受け付け、利用者の本人確認を行う
    2. 管理者がパスワードをリセットし、利用者に仮りパスワードを伝える
