...
- JavaScriptのライブラリの慣習として最後の引数で、ライブラリからの返り値を引数としたアロー関数を渡している
  - 例
    - サーバとのやりとりがあるときには、第二引数に「リクエストとレスポンスを引数とするアロー関数」を書いて、リクエストとレスポンスの値を受け取る
    - DBのやりとりがあるときには、第二引数にDBからの返り値である、「errとrow」が引数のアロー関数を渡している
  - `app.get`や`db.get`の内部で、第二引数に渡されたアロー関数を呼び出して、`req, res`などの引数に内部で値を格納して、それに対してユーザが書いた処理を行っていると考えられる
