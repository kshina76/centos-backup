...
![cpu-2](https://user-images.githubusercontent.com/53253817/102005329-62923780-3d5b-11eb-8a46-7db88d55eab8.png)
...
- ロードバランシングしたい時は、TCPレベルのロードバランシングをしないと意味がない
  ...
  - https://www.slideshare.net/AmazonWebServicesJapan/socket-15753751
...
- ブラウザの論理解像度をCSSピクセル、論理解像度と物理解像度の比率をピクセルデバイスレシオと呼ぶ
  - `srcset`を使って画像の解像度を合わせる設定をするとレスポンシブデザインでもきれいに画像を表示することができる
...
- URIで識別されるエンティティ間の関係性を記述する(主語、述語、目的語)
#### 8-2-2. ダブリンコア
- メタデータ記述のボキャブラリー集(RFC5013)
...
#### 8-2-4. マイクロフォーマット
- RDFとは異なり、HTMLのタグとクラスを使い表現する
- 一時期もてはやされたが、CSSのクラス名衝突でレイアウトが崩れるなど問題あり、現在ではschema.orgのデータフォーマットが推奨されている
#### 8-2-5. マイクロデータ
- HTMLに埋め込み可能なセマンティックの表現形式(W3C定義) - ボキャブラリーはマイクロフォーマットよりもかなり少ない(ex. Event,Organization,Person,Product,...)
#### 8-2-6. RDFの逆襲
- RDFそのものは広く使われなかったが、様々な派生フォーマットを生み出した(ex. RDF/XML,Turtle,JSON-LD,RDFa,...)
...
- オープングラフプロトコルが設定されているwebサイトに、URLを貼り付けるとURLだけでなく、記事の内容が一部引用された形で表示するやつ
...
- 高速化の仕組み
#### 8-5-1. ページの構成を固定化
- ルールで決められているサブセット、もしくは特別な代替記法のみをつかう制約を設ける
#### 8-5-2. CDN対応
- クローラがAMPをに意識すると、その内容をすべてCDNにコピーする
#### 8-5-3. タグの最適化
- コンテンツをサーバ側に引き取るときにタグを書き換える(仕様外,Googleの独自実装)
...
## 10章 クライアント視点でみるRESTful API
- サーバ側の開発者視点で設計するのは「Web API good parts」といった書籍を見る
### 10-1. APIとRPCの違い

## 12章 Webアプリケーションの基礎
### セッション管理をするためのストレージは三種類ある
- 多くのWebアプリケーションフレームワークでは、セッションストレージという名前で提供されている
1. クッキー保存
  - ブラウザに直接ユーザの情報を保存する
2. DB保存(RDB, NoSQL, Memcached)
  - ブラウザにID
  - DBにデータ
3. メモリ保存
  - ブラウザにID
  - サーバのメモリにデータ
### 12-4. Webアプリケーションの動作パターン
#### 12-4-1. 第1世代: サーバサイドレンダリング
- データベースで取得したデータをHTMLに流し込んで完成したHTMLをクライアントにレスポンスとして返却する方法
- 要はサーバ側がページの更新を行うような方法
- golangだけで作ったりするとこの方式のWebアプリケーションを作っていることになる
- 348画像
#### 12-4-2. 第2世代: Ajax
- クライアント側のJavaScriptが動的に画面の更新を行うことなく、ページに変化を加える方法
- XMLHttpRequestという技術を使って実現している
- Ajaxがアクセスする先のAPIのサーバのパターン
  1. HTMLなどを配信したのと同じサーバーで提供するパターン
    - 特定のURLのパス以下がWebAPIのパスとなっているもの
    - `example.com/api`以下がWebAPIのパスといった感じ
  2. `api.example.com`のように別のサブドメインでAPIを提供するパターン
  3. 全く別のドメインで提供するパターン
- 最初に読み込むHTMLはサーバサイドレンダリングを使って生成するパターンや、完全に静的なHTMLファイルのパターンもある
- 349画像
#### 12-4-3. 第3世代: シングルページアプリケーション(SPA)
- フロントエンドの内部でHTMLを生成するようにした方法
- 350画像
#### 12-4-4. 第3.5世代: シングルページアプリケーション + サーバサイドレンダリング
- SPAはページ遷移は高速だけど、初回のロードに時間がかかるという問題点があった
  - 大量のJavaScriptのライブラリを読み込んでこなくてはいけなかったから
- 上記の問題を解決するためにサーバサイドレンダリングを併用する方法が提唱された
- Next.jsやNuxt.jsを使うとこの併用機能を用いることができる
- 352画像
### 12-5. APサーバの構成
#### 12-5-1. GIL
- インタプリタ言語にはGIL(Global Interpreter Lock)という機構が備わっていて、プロセス内で実行できるスレッドが一つに制限されるという問題があった
  - これに対応するために複数のプロセスを起動することで、複数のスレッドが走っているようにする機構が使われる
...
- 上記はDNSでの名前解決からリクエストが飛んでいくまでの図
- 「DNS resolver」がDNSサーバのキャッシュをするところ
  - AWSだとVPCにAWS DNSとして付属しているみたい
#### 13-2-3. DNSクライアントのキャッシュ
- ブラウザは短期間だけDNSの結果をキャッシュする
- Keep-Alive機能が標準だからWebサーバのDNSの再問い合わせ起こらないが、追加のコンテンツを取ったり、遅れてAjax通信をする場合にDNSの問い合わせが起こるので、その際に効果がある
#### 13-2-4. DNSを使ったロードバランシング
- IPアドレスとドメイン名の対応づけはDNSのAレコードというところに入っている
- 一つのドメイン名に複数のIPアドレスを割り振ることでDNSラウンドロビンという負荷分散を実現できる
  - Aレコードを複数設定することで実現
- AWS ELBは多段でロードバランシングしていて、一段目はDNSラウンドロビンで負荷分散している

  ![elbconfguide01](https://user-images.githubusercontent.com/53253817/102009648-376c1000-3d7c-11eb-91a7-2fbeff101615.png)
- https://dev.classmethod.jp/articles/elb-configuration-guide-1/

### 13-3. リバースプロキシ
- リバースプロキシはトポロジ上の位置を表しているだけで、機能によって名前が変わる
- エッジそばでコンテンツをキャッシュして配信するのリバースプロキシをCDN
- 負荷分散をするリバースプロキシはロードバランサー
- TLSでリクエストを受けて暗号化を行い、裏にはHTTPで投げるというハンドシェイクを行うのもフロントのリバースプロキシ
- 認証機能を提供したり、コンテンツを圧縮したり、ログを出したり、静的コンテンツの配信をしながら動的コンテンツは裏のサーバに流したり

### 13-4. CDN
- 通信そのものの高速化
- ユーザに近い高機能なプロキシサーバ
- CDNを導入したことによって、日本からアメリカにあるサーバへのアクセス時間が半減したという報告もある
#### 13-4-1. 
