...
- アプリケーションアーキテクチャの構成
    - レイヤー構成
        - 3層、純粋なレイヤードアーキテクチャ、トランザクションスクリプト
    - presentation層
        - ハンドラを定義
        - ViewとControllerの役割
        - usecase層のinterfaceを持つ
    - usecase層
        - modelはプロパティのみでクラス定義
        - infraとのやりとりのためのDTOを定義
        - infra層にアクセスするためのクラスを別途定義
            - infra層のinterfaceをプロパティに持つ
    - infra層
        - usecaseとのやり取りのためにDTOを定義
        - DDDなので、DIPを使用することによりdomainに依存させる
        - SQL発行
...
- わかったことver2
    - htmlとcssを先に書いてしまうときには、templateも一度に開発する。その際に、バックエンド側ではハンドラとモックデータを使う。ここで、データベースアクセスとかを考えてしまうと、頭が働かなくてなってしまうので注意。
        - モックデータは実際に使うときの構造体のプロパティは揃えておくこと。でないとtemplateの細かい部分を書き直すことになってしまう
    - 「1ページ目のhtml->1ページ目のCSS」、「2ページ目のhtml->2ページ目のCSS」というサイクルを回すといいかも
    - marginの範囲とかは、safariやchromeの開発者ツールで見ながら調整すると効率がいい
    - プログラムを組む手順
        - 同時に複数のことをやらないことが一番効率がいい
        - https://note.com/erukiti/n/nec8e3dbe4a53
    - レイヤードアーキテクチャを組むときの手順
        - 下位層から上位層に向かって実装する
    ...
    - レイヤードアーキテクチャの力を知った
        - 機能追加した時の変更箇所がかなり少ない
            - 変更の被害が出るのは、変更箇所の一つ上のレイヤーだけで済んだ
    - メソッドの実装を書くときには、入力と出力を意識して開発を進める
        - 例えば、入力はユーザ入力で出力は次の層に入力を伝搬させるためのDTOとか意識しておくとわかりやすい
    - レイヤードアーキテクチャで引数にUserとかSessionとかの構造体を渡すようにしたほうが、依存先のメソッドの実装を意識しなくていいから開発がしやすい気がする
    - 依存先のメソッドの引数とか返り値をファイル遷移で見に行くのではなくて、vscodeで返り値と引数を見るようにすると開発が捗る
    - golangの独自エラーを活用する
        - https://qiita.com/ryskiwt/items/7dbdb937889623f77fd8
