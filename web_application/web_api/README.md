...
![2020-12-17 3 14のイメージ (1)](https://user-images.githubusercontent.com/53253817/102389119-07d83480-4016-11eb-885e-3ff05707279c.jpeg)
...
- Cache-Controlヘッダーにno-cacheというプロパティを与える
- 用途としては、非常に高頻度でデータが変わって、その変わったデータが影響を及ぼす場合にはキャッシュはして欲しくないから、no-cacheを使う
- no-cacheはキャッシュをしないのではなくて、検証モデルを使ってキャッシュするという意味。
  - 検証モデルを使えば、データが古いということはないから
- 本当にキャッシュをさせたくない場合はno-storeを使う
#### 4-3-5. Varyでキャッシュの単位を指定する
- Varyヘッダーはキャッシュサーバーに「同じURLでもVaryヘッダーに指定されているヘッダーの内容が異なれば別オブジェクトとして取り扱いなさい」というオリジンサーバーからの命令
- HTTPのAcceptで始まるヘッダーやUser-Agentヘッダーの値によってレスポンスの内容が異なるケースがあるから
- 異なるパターンは以下にまとまっている
  - https://qiita.com/AtsushiFukuda/items/2048e4b9f35b761d822d
#### 4-3-6. Cache-Controlヘッダ
- Cache-Contrplヘッダにprivateやpublicを指定することで、他のユーザにキャッシュの内容が見られないようになる(多分重要)
- 他にも色々な値があるので出てきたら適宜調べるといいかも

### 4-4. メディアタイプの指定
- Content-Type
#### 4-4-1. メディアタイプを指定することの重要性
- しっかり設定しないと、クライアント側が正しくデータを読むことができなくなる可能性があるから
#### 4-4-2. 「x-」で始まるメディアタイプ
- allication/x-msgpack のようなもの
- あまり使用されないもの
- application/x-www-form-urlencoded だけは例外
#### 4-4-3. 自分でメディアタイプを定義する場合
- application/vnd.ms-excel のように「.」で定義する
#### 4-4-4. 標準化されたデータ形式を使って独自のデータ形式を定義
- application/rss+xml のようにxmlをベースにrssを定義している
- ただし、これだとわかりにくいので、Content-Typeはxmlだけど、X-Github-Media-Typeのような独自ヘッダーで細かいタイプをしているといった方法が取られる
#### 4-4-6. リクエストとメディアタイプ
- リクエストの場合は、Content-TypeとAcceptヘッダがよく使われる
  - Acceptはブラウザが対応しているメディアタイプを送っている
- 詳しい内容はP129,130に書いてある

### 4-5. 同一生成元ポリシーとクロスオリジンリソース共有
- XMLHttpRequestは基本的に異なるドメインにアクセスはできない
  - 同一生成元ポリシーというセキュリティによって
  - 生成元はURIの中のスキーム、ホスト部、ポートで判断される
- アクセスの許可にはCORSを使う
  - IE8,IE9はXDomainRequestを使う必要がある
#### 4-5-1. CORSの基本的なやりとり
1. originヘッダーにアクセス元となる生成元を指定する
  - 例えば、http://www.example.com/からhttp://api.example.com/ へのアクセスをしたかったら、Originにhttp://www.example.com/ を指定する
2. サーバ側では、アクセスを許可する生成元の一覧を保持して判定する
  - 含まれていないなら403エラー
  - 含まれているなら、Access-Control-Allow-Originに生成元のURIを入れて返す
- どこからでも許可するなら「Access-Control-Allow-Origin: *」と指定する
  - 一般公開のAPIならこの設定。フロントようならちゃんと設定するのかな？
#### 4-5-2. プリフライトリクエスト
- CORSに定義されているサーバへの特別な問い合わせ方法
- 生成元をまたいだリクエストを行う前にそのリクエストが受け入れられるかどうかを事前にチェックするもの
- OPTIONメソッドで送られる
