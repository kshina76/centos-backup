...
- それぞれのレート帯でやったこと
  - https://emtubasa.hateblo.jp/entry/2019/03/31/211029
...
- 区間スケジューリング問題を解く手順
  ...
  - 3.重なっているものを削除
  - 4.2に戻って繰り返す
- 本に書いてあったことの解説(貪慾法が適用できる理由)

  ![image0-2](https://user-images.githubusercontent.com/53253817/104685969-976d2200-573f-11eb-8c1d-08dfae213b01.jpeg)

### 貪欲法パターン2: 現在がよいほど未来もよい
### この章の自分的まとめ
- 貪欲法が適用できるかどうかの判断
  - 実際の数値を使って紙で「その場での最善」を選択する貪欲法で解いてみて、「区間スケジューリング問題」のように交換可能な性質がありそうなら貪欲法が有効な可能性が高い
  - 一見バラバラな構造でもソートをして単調な構造に変化させると貪欲法が適用できることがある
  - 凸構造の場合に貪欲法は上手くいくらしい.
- 貪欲法は局所最適化の一種で、「局所最適解 = 最適解」の場合に確実に解けるという方法ということがわかった
  - 凸構造の場合には「局所最適解 = 最適解」なので、貪欲法で解ける
  - 凸凹構造の場合には「局所最適解 != 最適解」なので近似しかできない
    - ナップサック問題などは凸凹構造
- 貪欲法も大きい問題を複数の小問題に分割して、一つ一つを最適化することで大きい問題を解く方法
  - https://qiita.com/m1t0/items/b6bc511b50f1c9c1edd8
...
- 木: グラフが「連結」かつ「サイクルを持たない」構造
- 順序木: 各頂点vの子頂点の順序を考慮した木
- 二分木: 子頂点の数が高々2つの木
- 強平衡二分木: 「二分木」かつ「全ての葉の高さの違いが1以内」を満たす木
- 完全二分木: 「二分木」かつ「全ての葉の高さが等しい」を満たす木
  - 強平衡二分木のさらに特殊なケース
### グラフの実装
- https://qiita.com/ell/items/2a327fe021fb7dafe07a#入力-to-隣接リスト
### 重みつきグラフの実装
- https://qiita.com/ell/items/2a327fe021fb7dafe07a#入力-to-隣接リスト
### 二分木を用いるデータ構造1: ヒープ
- 定義
  - 頂点vの親頂点をpとしたとき、`key[p] >= key[v]`を満たす
  - 木の高さをhとしたとき、木の深さh-1以下の部分については、完全二分木を形成している
  - 木の高さをhとしたとき、木の深さhの部分については、頂点が左詰めされている
- 定義より強平衡二分木を満たしていることがわかる

![Max-heap](https://user-images.githubusercontent.com/53253817/104692004-8f66af80-574a-11eb-8ac0-72ffcac3c565.png)

### 二分木を用いるデータ構造2: 二分探索木
- あとで
### 二分木を用いるデータ構造3: 平衡二分探索木
- 実現方法
  - 赤黒木
  - AVL-木
  - B-木
